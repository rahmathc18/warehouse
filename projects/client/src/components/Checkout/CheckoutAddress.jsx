...
export const CheckoutAddress = ({ setDestination }) => {
  ...
  const fetchAddress = useCallback(async () => {
    try {
      ...
      setDestination(
        result.data.result.find((item) => item.default_address === true).city_id
      );
    } catch (err) {}
  }, [url, token]);
  ...
  return (
    <Box>
      {address.filter((item) => item.default_address === true)?.length ? (
            ...
            return (
              <>
                <Box
                  key={index}
                  borderRadius={"md"}
                  bgColor={item?.default_address ? "#90e0ef" : "#ade8f4"}
                  mt={{ base: "6", md: "3" }}
                  pl={4}
                  pt={2}
                  pb={4}
                ...
                >
                  <Box pt={2} display={"flex"} gap={2}>
                    <Badge
                      ...
                      colorScheme={"gray"}
                      pt={1}
                      hidden={item?.default_address ? false : true}
                    >
                      Main Address
                    </Badge>
                  </Box>
        ...
        <>
          <Text fontWeight={"500"}>You Don't Have a Main Address</Text>
          <Button
            onClick={onOpen}
            colorScheme="white"
            color="black"
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody mb={10}>
            ...
            >
              <Box>
                <AddAddress address={address} baseApi={url} name={name} />
              </Box>
            </Flex>
                ...
                address.map((item, index) => {
                  return (
                    ...
                    >
                      ...
                      <Box mb={3} color={"#black"} display={"flex"} gap={4}>
                        <EditAddress
                          address={address}
                          baseApi={url}
                          item={item}
